...
const functionGetNumber = () => new Promise((resolve, reject) => {
    fetch(`https://sms-activate.org/stubs/handler_api.php?api_key=${keyOtp}&action=getNumber&service=ot&operator=&country=6`, { 
        method: 'GET'
    })
    .then(res => res.text())
    .then(result => {
...
const functionChangeConfirm = (idOrder) => new Promise((resolve, reject) => {
    fetch(`https://sms-activate.org/stubs/handler_api.php?api_key=${keyOtp}&action=setStatus&status=6&id=${idOrder}`, { 
        method: 'GET'
    })
    .then(res => res.text())
    .then(result => {
